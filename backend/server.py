...
Bilingual AI Customer Support Voice Agent with optimized Romanian TTS
"""
...
db = client[os.environ.get('DB_NAME', 'voice_agent')]
...
# Create the main app
app = FastAPI(title="Bilingual Voice Agent - Natural Romanian TTS")

# Initialize voice agent services
...
def should_fetch_website_context(
    ...
    markers = [
        "site", "website", "menu", "pricing", "price", "product", "products",
        "restaurant", "reservation", "rezervare", "detalii", "details",
    ]
...
@api_router.get("/")
async def root():
    return {"message": "Voice Agent API - Natural Romanian TTS"}
...
async def health() -> dict:
    return {
        ...
        "tts_provider_ro": settings.tts_provider_ro,
...
@api_router.post("/simulate-turn", response_model=SimulateTurnResponse)
...
@api_router.post("/actions/research")
...
# Voice Settings endpoint - for tuning
@api_router.get("/voice-settings")
async def get_voice_settings() -> dict:
    """Get current voice settings for Romanian TTS"""
    return {
        "tts_provider": settings.tts_provider_ro,
        ...
        "style": settings.elevenlabs_style,
        "use_speaker_boost": settings.elevenlabs_use_speaker_boost,
        "behavior_style_ro": settings.behavior_style_ro,
        "recommendations": {
            "for_more_natural": "Increase stability (0.6-0.7), keep style at 0.3-0.5",
            "for_more_expressive": "Increase style (0.5-0.7), lower stability slightly",
            "for_clearer_speech": "Enable speaker_boost, increase stability"
        }
    }

# Status check routes (from original)

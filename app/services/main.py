...
async def build_smart_sms(
    ...
    """
    Genereaza un SMS complet contextual folosind LLM-ul.
    LLM-ul citeste intreaga conversatie si decide singur ce sa includa si cum sa formuleze.
    """
    from app.config import settings
    ...
    if language == "ro":
        prompt = (
            f"Citeste conversatia de mai jos si scrie un SMS scurt de confirmare sau rezumat, "
            f"exact asa cum ar scrie un om real de la {business_name}. "
            f"Include tot ce e relevant din conversatie — serviciu, data, ora, nume, detalii specifice. "
            f"Nu folosi template-uri sau fraze robotice. Maxim 3 randuri. "
            f"Scrie doar textul SMS-ului, nimic altceva.\n\n"
            f"Conversatie:\n{transcript}\n\nSMS:"
        )
    else:
        prompt = (
            f"Read the conversation below and write a short SMS confirmation or summary, "
            f"exactly as a real person from {business_name} would write it. "
            f"Include everything relevant from the conversation — service, date, time, name, specific details. "
            f"No templates or robotic phrases. Maximum 3 lines. "
            f"Write only the SMS text, nothing else.\n\n"
            f"Conversation:\n{transcript}\n\nSMS:"
        )
    ...
    if not api_key:
        # Fallback simplu daca nu e LLM configurat
        return _fallback_sms(language, history, business_name)
...
def _fallback_sms(language: str, history: list[dict[str, str]], business_name: str) -> str:
    """Fallback simplu daca LLM nu e disponibil."""
    recent = " | ".join(t["text"] for t in history[-4:] if t["role"] == "user")[:200]
    if language == "ro":
        return f"Confirmare {business_name}: {recent}. Raspunde la acest SMS pentru modificari."
    return f"Confirmation {business_name}: {recent}. Reply to this SMS for changes."
...
async def build_turn_response(session_id: str, user_text: str) -> SimulateTurnResponse:
    ...
    if sms_target and wants_sms(user_text):
        ...
        sms_message = await build_smart_sms(
            detection.language,
